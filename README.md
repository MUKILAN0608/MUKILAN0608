...
1. Save the workflow below as `.github/workflows/snake.yml` in your `MUKILAN0608/MUKILAN0608` profile repo.
2. Commit and push it to `main`.
3. Go to the **Actions** tab → select the workflow → click **Run workflow** (the first run won't trigger automatically off a schedule).
4. Once it finishes, check that a branch named `output` was created in the repo, containing `github-contribution-grid-snake.svg` and the `-dark.svg` variant.
5. The `<picture>` block in this README already points at that branch/path — once the branch exists, the image resolves on its own (no README edit needed).
...
<details>
<summary><code>$ rm -rf bugs/</code></summary>
<br>

```
rm: cannot remove 'bugs/': Permission denied.
Bugs have root access. Negotiations ongoing.
```
